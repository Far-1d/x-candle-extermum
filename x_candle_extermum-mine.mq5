...
input int lot_multiplier_win = 10;                  // % Percent to Increase Lot Size in Streaks
input int lot_multiplier_lose= 20;                  // % Percent to Decrease Lot Size in Streaks 
...
//+------------------------------------------------------------------+
//| from journal.mqh -> check if position number already exists      |
//+------------------------------------------------------------------+
int CheckMatch( string _position ){  
   int size = ( int )ArraySize(dataArray)/2;
   for (int i=0; i<size; i++)
   {
      if (dataArray[i][0] == _position && dataArray[i][1] == _Symbol)
      {
         return i;
      }
   }
                        ...
                        const MqlTradeResult& result)
{
   ...
   if(type == TRADE_TRANSACTION_DEAL_ADD) 
   {
      ...
      if (pos == -1)
      {
         int size = ArraySize(dataArray)/2;
         ArrayResize(dataArray, size+1);
         
         dataArray[size][0] = (string)trans.position;
         dataArray[size][1] = trans.symbol;
      }
      else
      {
         ...
         if (profit > 0)
         {
            // last position was profitable
            consecutive_wins ++;
            consecutive_loses = 0;
         }
         else
         {
            // last position was a loss
            consecutive_loses ++;
            consecutive_wins = 0;
         }
      }
...
void check_bars(){
   ...
   if (bar_size >= calculated_box_size*box_percent/100 && calculated_box_size > 0 && bar_size != 0)
   {
      //--- calculate lot size
      double multiplier = 1;
      if (consecutive_loses==0 && consecutive_wins==0){}
...
void trailing(ulong tikt , string type){
   ...
   if (PositionGetString(POSITION_COMMENT) == "trail")
   {
